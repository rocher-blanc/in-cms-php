...
$app->group('/menu', function () use ($app)
{
    ...
    $app->get('/element/order/down/:id/:token', function ( $id ) use ($app)
    {
        $ret  = true ;
        $msg  = 'La position de l\'élément a bien été modifié' ;

        $elm = \DB::for_table('menu_element')
            ->where_equal('menu_element_id',$id)
            ->find_one();

        $ct = \DB::for_table('menu_element');
        if ( $elm->menu_element_parent_id === NULL )	$ct = $ct->where_null('menu_element_parent_id') ;
        else											$ct = $ct->where([ 'menu_element_parent_id' => $elm->menu_element_parent_id ]) ;
        $ct = $ct->count();

        if ( $elm->menu_element_order == $ct )
        {
            $ret  = false ;
            $msg  = 'Impossible, l\'élément est déja au plus bas' ;
        }
        else
        {
            $newpos = $elm->menu_element_order + 1;

            $top = \DB::for_table('menu_element');
            if ( $elm->menu_element_parent_id === NULL )	$top = $top->where([ 'menu_element_order' => $newpos ])->where_null('menu_element_parent_id') ;
            else											$top = $top->where([ 'menu_element_order' => $newpos , 'menu_element_parent_id' => $elm->menu_element_parent_id ]) ;
            $top = $top->find_one();
            //\Debug::sql();
            $top->menu_element_order = $top->menu_element_order - 1;
            $top->save();

            $elm->menu_element_order = $newpos ;
            $elm->save();
        }

        $Factory = \App\Kernel\Factory::getInstance() ;
        $Factory->Response()->returnJSON( $msg , $ret ) ;
    })->name('menu_down_element');

    $app->get('/element/order/up/:id/:token', function ( $id ) use ($app)
    {
        $ret  = true ;
        $msg  = 'La position de l\'élément a bien été modifié' ;
        ...
        if ( $elm->menu_element_order == 1 )
        {
            $ret  = false ;
            $msg  = 'Impossible, l\'élément est déja au plus haut' ;
        }
        else
        {
            $newpos = $elm->menu_element_order - 1;
